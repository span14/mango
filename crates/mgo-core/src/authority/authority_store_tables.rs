...
mod tests {
    use super::*;
    use mgo_types::{
        base_types::{random_object_ref, MgoAddress, ObjectID, SequenceNumber, TransactionDigest, ExecutionDigests},
        ...
        messages_checkpoint::{CheckpointSummary, CheckpointContents, VerifiedCheckpoint, CertifiedCheckpointSummary},
        crypto::{AuthoritySignInfo, AuthoritySignature, MgoAuthoritySignature},
    };
    ...
    fn insert_test_data_for_epoch(
        ...
        let (mut test_committee, keys) = Committee::new_simple_test_committee();
        ...
        for _i in 0..num_transactions {
            ...
            let execution_digest = ExecutionDigests::new(tx_digest, effects_digest);
            ...
            execution_digests.push(execution_digest);
        }
        ...
        // Create and insert a checkpoint for this epoch into the checkpoint store
        let previous_digest = if epoch == 0 {
            None
        } else {
        ...
        let checkpoint_summary = CheckpointSummary::new(
            ...
            &CheckpointContents::new_with_digests_only_for_tests(execution_digests),
            previous_digest,
            GasCostSummary::default(),
        ...
        test_committee.epoch = epoch;
        
        // Create signatures from validators
        let signatures: Vec<AuthoritySignInfo> = test_committee
            .voting_rights
            .iter()
            .zip(keys.iter())
            .map(|((name, _), key)| {
                ...
                let signature = AuthoritySignature::new_secure(&intent_msg, &checkpoint_summary.epoch, key);
                AuthoritySignInfo {
        ...
        let certified_checkpoint = CertifiedCheckpointSummary::new(
            checkpoint_summary, 
            signatures, 
            &test_committee
        ).unwrap();
        ...
        let checkpoint = VerifiedCheckpoint::new_unchecked(certified_checkpoint);
        
        // Insert the checkpoint into the checkpoint store
        checkpoint_store.insert_verified_checkpoint(&checkpoint).unwrap();
        checkpoint_store.insert_epoch_last_checkpoint(epoch, &checkpoint).unwrap();
    ...
    async fn test_rollback_to_epoch_basic() {
        ...
        let epoch1_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 1, 2);
        let epoch2_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 2, 3);
        let epoch3_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 3, 2);
    ...
    async fn test_rollback_to_epoch_zero() {
        ...
        let epoch1_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 1, 2);
        let epoch2_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 2, 1);
    ...
    async fn test_rollback_validation_prevents_future_epoch() {
        ...
        insert_test_data_for_epoch(&store, &checkpoint_store, 1, 1);
    ...
    async fn test_rollback_atomic_operation() {
        ...
        // Insert data for epochs 1 and 2
        let epoch1_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 1, 1);
        let epoch2_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 2, 1);
    ...
    #[tokio::test]
    async fn test_rollback_singleton_tables() {
        let (store, checkpoint_store, _temp_dir) = create_test_authority_store();

        // Set initial singleton values
        store.expected_network_mgo_amount.insert(&(), &10_000_000_000).unwrap();
        store.expected_storage_fund_imbalance.insert(&(), &1000).unwrap();
        store.pruned_checkpoint.insert(&(), &50).unwrap();

        // Insert data for epochs 1, 2, 3
        let _epoch1_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 1, 1);
        let epoch2_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 2, 2);
        let epoch3_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 3, 1);

        // Verify initial epoch_start_configuration is at epoch 3
        let initial_config = store.epoch_start_configuration.get(&()).unwrap().unwrap();
        assert_eq!(initial_config.epoch_start_state().epoch(), 3);
        ...
        // Check singleton tables after rollback
        
        // 1. epoch_start_configuration should be updated to epoch 2
        let config_after_rollback = store.epoch_start_configuration.get(&()).unwrap().unwrap();
        assert_eq!(config_after_rollback.epoch_start_state().epoch(), 2, "epoch_start_configuration should be rolled back to epoch 2");

        // 2. pruned_checkpoint should be updated
        let pruned_checkpoint_after = store.pruned_checkpoint.get(&()).unwrap().unwrap();
        assert_eq!(pruned_checkpoint_after, 100, "pruned_checkpoint should be updated to checkpoint 100 (epoch 1 last checkpoint)");

        // 3. expected_network_mgo_amount should remain unchanged (it's a constant)
        let network_mgo_after = store.expected_network_mgo_amount.get(&()).unwrap().unwrap();
        assert_eq!(network_mgo_after, 10_000_000_000, "expected_network_mgo_amount should remain unchanged");

        // 4. expected_storage_fund_imbalance should be adjusted
        // Since we removed transactions with default gas cost summary (0 storage rebate),
        // the imbalance should remain the same (1000 + 0 = 1000)
        let imbalance_after = store.expected_storage_fund_imbalance.get(&()).unwrap().unwrap();
        assert_eq!(imbalance_after, 1000, "expected_storage_fund_imbalance should be adjusted correctly");
    }

    #[tokio::test]
    async fn test_rollback_singleton_with_storage_rebates() {
        let (store, checkpoint_store, _temp_dir) = create_test_authority_store();

        // Set initial singleton values
        store.expected_storage_fund_imbalance.insert(&(), &5000).unwrap();

        // Create transaction effects with non-zero storage rebates
        let tx_digest = TransactionDigest::random();
        let mut gas_summary = GasCostSummary::default();
        gas_summary.storage_rebate = 2000; // Non-zero storage rebate
        ...
        let verified_tx = VerifiedTransaction::new_genesis_transaction(vec![]);
        let trusted_tx = verified_tx.serializable();
        store.transactions.insert(&tx_digest, &trusted_tx).unwrap();
        store.effects.insert(&effects_digest, &effects).unwrap();
        store.executed_effects.insert(&tx_digest, &effects_digest).unwrap();

        // Setup epochs
        store.root_state_hash_by_epoch.insert(&1, &(100, Accumulator::default())).unwrap();
        store.root_state_hash_by_epoch.insert(&2, &(200, Accumulator::default())).unwrap();

        // Rollback to epoch 1, removing the transaction with storage rebate
        let batch = store.rollback_to_epoch(1, &checkpoint_store, &vec![tx_digest], &vec![effects_digest]).unwrap();
        batch.write().unwrap();

        // Check that storage fund imbalance was adjusted by adding back the removed storage rebate
        let imbalance_after = store.expected_storage_fund_imbalance.get(&()).unwrap().unwrap();
        assert_eq!(imbalance_after, 7000, "expected_storage_fund_imbalance should be increased by removed storage rebate (5000 + 2000)");
    }

    #[tokio::test]
    async fn test_rollback_preserves_singleton_table_consistency() {
        let (store, checkpoint_store, _temp_dir) = create_test_authority_store();

        // Set all singleton values
        store.expected_network_mgo_amount.insert(&(), &10_000_000_000).unwrap();
        store.expected_storage_fund_imbalance.insert(&(), &0).unwrap();
        store.pruned_checkpoint.insert(&(), &0).unwrap();
        
        // Insert data for multiple epochs
        let _epoch1_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 1, 2);
        let epoch2_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 2, 3);
        let epoch3_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 3, 2);
        let epoch4_txs = insert_test_data_for_epoch(&store, &checkpoint_store, 4, 1);

        // Verify initial state
        assert!(store.root_state_hash_by_epoch.get(&4).unwrap().is_some());
        let initial_config = store.epoch_start_configuration.get(&()).unwrap().unwrap();
        assert_eq!(initial_config.epoch_start_state().epoch(), 3);

        // Perform multiple rollbacks and verify singleton consistency
        
        // First rollback: epoch 4 -> 3
        let txs_to_remove_4 = epoch4_txs.clone();
        let effects_to_remove_4 = epoch4_txs.iter()
            .map(|tx| store.executed_effects.get(tx).unwrap().unwrap())
            .collect();
        
        let batch = store.rollback_to_epoch(3, &checkpoint_store, &txs_to_remove_4, &effects_to_remove_4).unwrap();
        batch.write().unwrap();
        
        // Verify singleton tables after first rollback
        assert_eq!(store.epoch_start_configuration.get(&()).unwrap().unwrap().epoch_start_state().epoch(), 3);
        assert_eq!(store.pruned_checkpoint.get(&()).unwrap().unwrap(), 200); // epoch 2's last checkpoint
        assert_eq!(store.expected_network_mgo_amount.get(&()).unwrap().unwrap(), 10_000_000_000);
        
        // Second rollback: epoch 3 -> 1
        let txs_to_remove_2_3 = epoch2_txs.iter().chain(epoch3_txs.iter()).cloned().collect();
        let effects_to_remove_2_3 = epoch2_txs.iter().chain(epoch3_txs.iter())
            .map(|tx| store.executed_effects.get(tx).unwrap().unwrap())
            .collect();
        
        let batch = store.rollback_to_epoch(1, &checkpoint_store, &txs_to_remove_2_3, &effects_to_remove_2_3).unwrap();
        batch.write().unwrap();
        
        // Verify singleton tables after second rollback
        assert_eq!(store.epoch_start_configuration.get(&()).unwrap().unwrap().epoch_start_state().epoch(), 1);
        assert_eq!(store.pruned_checkpoint.get(&()).unwrap().unwrap(), 0); // epoch 0's last checkpoint
        assert_eq!(store.expected_network_mgo_amount.get(&()).unwrap().unwrap(), 10_000_000_000);
        
        // Verify only epoch 0 data remains
        assert!(store.root_state_hash_by_epoch.get(&1).unwrap().is_none());
        assert!(store.root_state_hash_by_epoch.get(&2).unwrap().is_none());
        assert!(store.root_state_hash_by_epoch.get(&3).unwrap().is_none());
        assert!(store.root_state_hash_by_epoch.get(&4).unwrap().is_none());
    }
}
